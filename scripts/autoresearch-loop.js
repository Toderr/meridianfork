...
function pickExtremes(perfRecords) {
  ...
  return {
    ...
    winCount: todayRecords.filter(r => (r.pnl_usd ?? 0) >= 0).length,
    lossCount: todayRecords.filter(r => (r.pnl_usd ?? 0) <  0).length,
  };
}
...
function formatTrade(label, trade) {
  ...
  const usd = trade.pnl_usd ?? 0;
  const fees = trade.fees_earned_usd ?? 0;
  const sign = pct >= 0 ? "+" : "";
  return `${label}:
  ...
  PnL: ${sign}${pct.toFixed(2)}% ($${usd.toFixed(2)})
  Fees earned: $${fees.toFixed(2)}
...
function buildNotificationMessage(extremes, newLessons, rationale) {
  ...
  const wPct = w.pnl_pct ?? 0;
  const lPct = l.pnl_pct ?? 0;
  const wSign = wPct >= 0 ? "+" : "";
  const lSign = lPct >= 0 ? "+" : "";

  const parts = [`🔬 DAILY RESEARCH`];
  parts.push(`Closes: ${extremes.totalCloses} | W ${extremes.winCount} / L ${extremes.lossCount}`);
  parts.push(`\n🏆 Best: ${w.pool_name} ${wSign}${wPct.toFixed(2)}% (${w.strategy}, ${w.minutes_held}m)`);
  parts.push(`💀 Worst: ${l.pool_name} ${lSign}${lPct.toFixed(2)}% (${l.strategy}, ${l.minutes_held}m)`);
...
export async function runDailyAutoresearch() {
  ...
  try {
    ...
    const logWinPct = extremes.win.pnl_pct ?? 0;
    const logLossPct = extremes.loss.pnl_pct ?? 0;
    log("autoresearch", `Daily review: ${extremes.totalCloses} closes | best: ${extremes.win.pool_name} +${logWinPct.toFixed(1)}% | worst: ${extremes.loss.pool_name} ${logLossPct.toFixed(1)}%`);
