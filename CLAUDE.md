...
- **Screening hard gates (index.js `candidateBlocks` filter)**: `swap_count >= 6` → reject (2026-05-08: boundary fixed from `> 6`; audit showed `=6` caused −8.37% loss via illiquid gap-through); `volume < $500` → reject (2026-05-08: illiquid pools cause extreme SL gap-through when closing a $70 position moves the market itself)
