...
6. **HARDCODED 90m hold cap (Rule 3b)**: `age ≥ 90m` → CLOSE **unless** fees/hr over the last 30m ≥ $4 **OR** fee-incl pnl ≥ +1.5% and age < 110m. Tightened from 120 → 90 on 2026-04-27 after post-rebuild audit (n=108) showed `120m+` bucket = 30.8% wr / −0.26% avg (n=13) and `60-120m` = 93.1% wr / +0.21% avg (n=58) — the sweet spot ends near 90m. Fee-rate escape ($4/hr over last 30m, raised from $2 on 2026-04-23) permits productive compounders to extend. **PnL-based escape added 2026-04-29** after audit since config change (n=40) showed 4/10 hard-cap kills were profitable @ avg +1.04% — fee rate dried up but fee-incl pnl remained positive. **Threshold tightened 0.5 → 1.0 on 2026-04-30** after 30h delta audit (n=21 in extend window): 8 profitable kills avg +1.24%, 13 negative kills avg −0.63% — extend at 0.5% peak became a give-back trap roughly half the time, net −4.8 pp. **Threshold raised 1.0 → 1.5 on 2026-05-08** after 5-day delta audit (n=108 in 90-120m bucket, wr=44.4%) confirmed extend window still giving back at 1.0% bar. Profitable positions extend up to 110m absolute ceiling. Computed from `getRecentFeeRate()` in `pool-memory.js`. Constants at top of `management-rules.js` — `HARD_HOLD_CAP_MIN`, `HARD_HOLD_FEE_WINDOW_MIN`, `HARD_HOLD_MIN_FEE_RATE_USD_HR`, `HARD_HOLD_PROFITABLE_PCT`, `HARD_HOLD_PROFITABLE_EXTEND_MIN`.
...
- **Screening hard gates (index.js `candidateBlocks` filter)**: `swap_count >= 6` → reject (2026-05-08: boundary fixed from `> 6`; audit showed `=6` caused −8.37% loss via illiquid gap-through); `volume < $500` → reject (2026-05-08: illiquid pools cause extreme SL gap-through when closing a $70 position moves the market itself); `okx_price_change_1h < -20%` → reject (2026-05-12: 25% SL rate, n=24 — token in active dump; only enforced when OKX data available); `momentum_buyers_1h ≥ 15 AND global_fees_sol < 80` → reject (2026-05-12: 17.9% SL rate, n=106 — FOMO entry on illiquid token; high buyer pressure + shallow liquidity = price peaked, one seller dumps through position)
